...
function walk(i){
  ...
  // Draw title on first iteration
  if(i===0){
    t.penup();
    t.goto([-90,-60]);
    txt.print(t,"POMFRET SCHOOL CAMPUS",0.15);
  }
  ...
  // Add building label with custom offset to avoid overlaps
  const c=centroid(p);
  const [labelX,labelY]=proj(c[0],c[1],h*1.2,i);
  const labelOffsetX=L[i][0];
  const labelOffsetY=L[i][1];
  t.penup();
  t.goto([labelX*S+offsetX+labelOffsetX,labelY*S+offsetY+labelOffsetY]);
  txt.print(t,N[i],0.08);
