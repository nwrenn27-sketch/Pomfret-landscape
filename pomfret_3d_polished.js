...
// Architectural details: foundations, cornices, mullions, eaves, chimneys, ground grid
...
const showLandscape=1;/// min=0 max=1 step=1
const showLabels=0;/// min=0 max=1 step=1
...
function walk(i){
  ...
  for(let j=0;j<p.length;j++){
    ...
    if(showDoors&&j===0){
      ...
      if(type==='chapel'&&detailLevel>1){
        const [a0,a1]=proj(p[j][0]+dx*dH,p[j][1]+dy*dH,dZ*1.15,i);
  ...
  // Roof ridge
  if(showRoofs){
    const mi=longestEdge(p);
    const rH=type==='chapel'?h*1.25:h*1.1;

    if(type==='chapel'){
      const [r0,r1]=proj((p[mi][0]+p[(mi+1)%p.length][0])*0.5,(p[mi][1]+p[(mi+1)%p.length][1])*0.5,rH,i);
      const j2=(mi+2)%p.length;
      const [r2,r3]=proj((p[j2][0]+p[(j2+1)%p.length][0])*0.5,(p[j2][1]+p[(j2+1)%p.length][1])*0.5,rH,i);
  ...
  if(showLabels&&N[i]){
    const c=centroid(p);
    const [lx,ly]=proj(c[0],c[1],h*1.15,i);
    const label=N[i],charW=2.5,charH=4,labelW=label.length*charW;
    t.jump([lx*S+offsetX-labelW/2,ly*S+offsetY-charH]);
    t.goto([lx*S+offsetX+labelW/2,ly*S+offsetY-charH]);

    for(let c=0;c<label.length;c++){
      const cx1=lx*S+offsetX-labelW/2+c*charW+charW/2,cy1=ly*S+offsetY;
      t.jump([cx1-0.5,cy1-0.5]);t.goto([cx1+0.5,cy1-0.5]);
      t.goto([cx1+0.5,cy1+0.5]);t.goto([cx1-0.5,cy1+0.5]);t.goto([cx1-0.5,cy1-0.5]);
    }
  }
  return true;
}
...
if(showLandscape){
  ...
  trees.forEach(tr=>{
    const [tx,ty]=proj(tr[0],tr[1],0,0),[ttx,tty]=proj(tr[0],tr[1],0.00003*tr[2],0);
    t.jump([tx*S+offsetX,ty*S+offsetY]);t.goto([ttx*S+offsetX,tty*S+offsetY]);
  });

  if(detailLevel>1){
    for(let cl=0;cl<3;cl++){
      const cy=-0.003+cl*0.0008,pts=[];
