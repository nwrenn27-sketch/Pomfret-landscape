// Pomfret School Campus - 3D Isometric View (Enhanced with Terrain)
// 12 core buildings + terrain contours + pathways - Experimental version
...
const showTerrain = 1; /// min=0 max=1 step=1 (No terrain, With terrain)
const showPaths = 1; /// min=0 max=1 step=1 (No paths, With paths)

let drawnTerrain = false;
...
function walk(i) {
  ...
  // Draw terrain and paths once before buildings
  if (i == 0 && !drawnTerrain) {
    if (showTerrain == 1) drawTerrain(t);
    if (showPaths == 1) drawPathways(t);
    drawnTerrain = true;
  }
...
function drawTerrain(t) {
  // Subtle elevation contours around campus
  const centerX = 0.0003;
  const centerY = -0.0008;

  for (let elev = 0; elev < 6; elev++) {
    const radius = 0.0012 + elev * 0.0004;
    const z = -elev * 0.00003;
    const pts = 64;
    const contour = [];
...
function drawPathways(t) {
  // Main pathways connecting key buildings
  const paths = [
    [[0.0005, -0.0001], [0.0002, -0.0010], [-0.0003, -0.0013]],
    [[-0.0006, 0.0004], [0.0004, 0.0002], [0.0012, -0.0017]],
    [[0.0005, 0.0002], [0.0006, 0.0006]],
    [[0.0003, -0.0018], [0.0009, -0.0024]]
  ];

  for (let path of paths) {
    const pathPts = [];
    for (let pt of path) {
      const [px, py] = proj(pt[0], pt[1], -0.00001);
      pathPts.push([px * S + oX, py * S + oY]);
    }
